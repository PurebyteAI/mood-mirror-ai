...
const JournalPage = ({ onBack }) => {
  ...
  const [activeView, setActiveView] = useState("entries"); // "entries" | "trends"
  ...
  const deleteEntry = async (entryId) => {
    try {
      ...
      setTrends((prev) => prev.filter((t) => t.timestamp !== entries.find((e) => e.id === entryId)?.timestamp));
    } catch (err) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 p-1 rounded-full" style={{ background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.1)" }}>
            <button
              ...
              style={{
                background: activeView === "entries" ? "rgba(255,255,255,0.1)" : "transparent",
                color: activeView === "entries" ? "var(--text-primary)" : "var(--text-muted)",
              }}
            >
            ...
            <button
              ...
              className="p-2 rounded-full transition-all"
              style={{
                background: activeView === "trends" ? "rgba(255,255,255,0.1)" : "transparent",
                color: activeView === "trends" ? "var(--text-primary)" : "var(--text-muted)",
              }}
            >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {getMoodStats(trends).map((stat) => (
                      <div
                        ...
                        data-testid={`stat-${stat.mood}`}
                      >
                        <div
                          className="w-3 h-3 rounded-full mx-auto mb-2"
                          style={{ background: MOOD_COLORS[stat.mood] || "#A1A1AA" }}
                        />
                        <p className="text-xs font-mono capitalize" style={{ color: "var(--text-muted)" }}>
                          {stat.mood}
                        </p>
                        <p className="text-lg font-bold font-mono mt-1" style={{ color: MOOD_COLORS[stat.mood] || "#A1A1AA" }}>
                          {stat.avgPct}%
                        </p>
                    ...
                    return (
                      ...
                      >
                        <div className="p-5">
                          ...
                          <p
                            className={`text-sm font-display leading-relaxed ${isExpanded ? "" : "line-clamp-2"}`}
                            style={{ color: "var(--text-secondary)" }}
                          >
                            {entry.response_text}
                          </p>
                          ...
                          {isExpanded && (
                            ...
                            >
                              <div className="px-5 pb-5 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                                ...
                                <MoodChart emotions={entry.emotions} />
                              </div>
                            </motion.div>
                          )}
                        </AnimatePresence>
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-24" data-testid="empty-journal">
    ...
    <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
      Save mood analyses to build your emotional timeline.
    </p>
  </div>
);
